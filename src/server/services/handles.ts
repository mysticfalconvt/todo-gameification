...
export async function handleExists(candidate: string, excludeUserId?: string): Promise<boolean> {
  const rows = await db
    .select({ id: user.id })
    .from(user)
    .where(
...
// error on conflict.
export async function generateUniqueHandleFromName(name: string): Promise<string> {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    const candidate = (usableBase.slice(0, 20 - suffix.length) + suffix).slice(0, 20)
