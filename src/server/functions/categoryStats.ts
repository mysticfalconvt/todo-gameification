...
import { getCategoryHistogramForUser, type CategoryScope } from '../services/categoryStats'
...
export const getCategoryHistogramFn = createServerFn({ method: 'POST' })
  ...
  .inputValidator((data: { targetUserId: string; scope: string }) => {
  ...
  .handler(async ({ data, context }) =>
    getCategoryHistogramForUser(context.userId, data.targetUserId, data.scope),
  )
...
export const getFriendsCategoryHistogramsFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const results = await Promise.all(
      entries.map(async (e) => {
        const hist = await getCategoryHistogramForUser(context.userId, e.userId, data.scope)
        return { ...e, ...hist }
      }),
    )
    return results
