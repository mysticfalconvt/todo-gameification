...
const METRICS: LeaderboardMetric[] = ['xp', 'avg-xp-day', 'streak', 'showed-up']
const WINDOWS: LeaderboardWindow[] = [7, 30, 90, 'all']

export const getLeaderboardFn = createServerFn({ method: 'POST' })
  .middleware([authMiddleware])
  .inputValidator((data: { scope: string; metric: string; days: number | string }) => {
    ...
    const days: LeaderboardWindow = data.days === 'all' ? 'all' : (Number(data.days) as 7 | 30 | 90)
  ...
  .handler(async ({ data, context }) => getLeaderboard(context.userId, data))
