    ...
    <main className="page-wrap space-y-6 px-4 py-8">
      <header>
        ...
        <p className="mt-2 max-w-2xl text-sm text-[var(--sea-ink-soft)]">
          Ready-to-paste config that surfaces Todo XP state as HA sensors and lets you complete /
          skip / snooze tasks from automations, dashboards, and physical buttons. Everything below
          assumes your app is at <code className="font-mono">todo.rboskind.com</code> — replace with
          your own domain if different.
        </p>
      </header>

      <section className="island-shell max-w-3xl space-y-3 rounded-2xl p-5">
        <h2 className="text-lg font-bold text-[var(--sea-ink)]">1. Mint an API token</h2>
        <ol className="list-decimal space-y-1 pl-5 text-sm text-[var(--sea-ink-soft)]">
          <li>
            ...
            → expand <strong>API access</strong> → create token named "Home Assistant".
          </li>
          <li>
            Copy the <code className="font-mono">tgx_…</code> value (shown once).
          </li>
          <li>
            Store it in HA's <code className="font-mono">secrets.yaml</code>:
          </li>
        </ol>
...
todo_xp_token: "Bearer tgx_xxxxxxxxxxxxxxxxxxxx"`}
        ...
        <p className="text-xs text-[var(--sea-ink-soft)]">
          Note the literal <code className="font-mono">Bearer&nbsp;</code> prefix — HA's{' '}
          <code className="font-mono">rest:</code> and{' '}
          <code className="font-mono">rest_command:</code> pass the secret through verbatim to the
  ...
  todo_xp_create_task:
    ...
    payload: >-
      ...
      <section className="island-shell max-w-3xl space-y-3 rounded-2xl p-5">
        <h2 className="text-lg font-bold text-[var(--sea-ink)]">4. Today's list as attributes</h2>
        <p className="text-sm text-[var(--sea-ink-soft)]">
          Scalar sensors are great for counts, but to render individual tasks on a Lovelace card you
          want the full list as attributes:
        </p>
        <pre className="overflow-x-auto rounded-md border border-[var(--line)] bg-[var(--surface-strong)] p-3 text-xs">
          {`rest:
  - resource: https://todo.rboskind.com/api/v1/today
...
content: |
  ...
  {% endfor %}`}
      ...
      <section className="island-shell max-w-3xl space-y-2 rounded-2xl p-5">
        <h2 className="text-lg font-bold text-[var(--sea-ink)]">5. Automations worth building</h2>
        <ul className="list-disc space-y-1 pl-5 text-sm text-[var(--sea-ink-soft)]">
          ...
          <li>
            <strong>On streak reset</strong> (currentStreak drops) → motivating push on a different
            channel.
          </li>
          <li>
            <strong>Physical kitchen button</strong> →{' '}
            <code className="font-mono">rest_command.todo_xp_complete_instance</code> on the first
            due instance. Hands-free "took meds".
          </li>
        </ul>
      </section>

      <section className="island-shell max-w-3xl space-y-2 rounded-2xl p-5">
        <h2 className="text-lg font-bold text-[var(--sea-ink)]">6. Troubleshooting</h2>
        <ul className="list-disc space-y-1 pl-5 text-sm text-[var(--sea-ink-soft)]">
          <li>
          ...
          <li>
            <strong>404 on complete/skip/snooze</strong> — instance IDs change every time a task
            materializes a new instance. Always read the current id from{' '}
            <code className="font-mono">sensor.todo_xp_today_list</code>, don't hard-code.
          </li>
          <li>
            <strong>Stale counts</strong> — default <code className="font-mono">scan_interval</code>{' '}
            is 300s; drop to 60s for snappier dashboards. Don't go below 30s.
          </li>
          <li>
            <strong>Rotate a token</strong> — revoke in settings, mint a new one, update{' '}
            <code className="font-mono">secrets.yaml</code>, HA core restart. Old token 401s
            immediately.
          </li>
        </ul>
      </section>
