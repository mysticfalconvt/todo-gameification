  ...
  return (
    <main className="page-wrap space-y-6 px-4 py-8">
      <header>
        ...
        <h1 className="display-title text-4xl font-bold text-[var(--sea-ink)]">MCP server</h1>
        <p className="mt-2 max-w-2xl text-sm text-[var(--sea-ink-soft)]">
          A Model Context Protocol server is mounted at <code className="font-mono">/api/mcp</code>.
          It exposes the same operations as the REST API as typed MCP tools, so Claude Desktop and
          other MCP clients can read and modify your tasks directly. Auth is the same{' '}
          <code className="font-mono">tgx_</code> token you use for REST.
        </p>
      </header>

      <section className="island-shell max-w-3xl rounded-2xl p-5">
        <h2 className="mb-2 text-lg font-bold text-[var(--sea-ink)]">Claude Desktop setup</h2>
    ...
    "todo-xp": {
      ...
      "headers": {
        ...
        <p className="mt-3 text-sm text-[var(--sea-ink-soft)]">
          Any MCP client with Streamable HTTP transport (URL + headers) works — open-interpreter,
      ...
      <section className="max-w-3xl space-y-3">
        ...
        <p className="text-sm text-[var(--sea-ink-soft)]">
          Every tool below is callable by a connected LLM. Arguments are typed (via Zod schemas),
          results are JSON.
        </p>
        ...
        <ToolCard
          ...
          args="none"
        />
        <ToolCard name="list_someday" purpose="Undated open tasks (no deadline)." args="none" />
        <ToolCard name="list_tasks" purpose="All active tasks the user has." args="none" />
        <ToolCard
          name="get_task"
...
function ToolCard({ name, purpose, args }: { name: string; purpose: string; args: string }) {
  return (
    <div className="island-shell rounded-xl p-4">
      <p className="font-mono text-sm font-semibold text-[var(--sea-ink)]">{name}</p>
