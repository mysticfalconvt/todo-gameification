    ...
    <main className="page-wrap space-y-6 px-4 py-8">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h1 className="display-title text-4xl font-bold text-[var(--sea-ink)]">Trends</h1>
        ...
        <div>
          ...
          {query.isLoading ? null : (
            <p className="mt-1 text-xs text-[var(--sea-ink-soft)]">
              {totalItems} completions · {totalXp} XP · {days.length} active{' '}
          ...
          {days.map((day) => (
            <li key={day.date}>
              ...
              <ul className="space-y-1">
                {day.items.map((item) => {
                  const rowClass = 'island-shell flex items-center gap-3 rounded-xl p-3'
                  const body = (
                    <>
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm text-[var(--sea-ink)]">{item.title}</p>
...
function MotivationSection({ days }: { days: Range }) {
  ...
  const completionRate =
    data.focus.started > 0 ? Math.round((data.focus.completed / data.focus.started) * 100) : null
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-4">
        ...
        />
        <StatCard label="XP from games" value={totalGameXp} hint={`${totalGamesPlayed} played`} />
      </div>
              ...
              {data.games.map((g) => {
                ...
                return (
                  <tr key={g.gameId} className="border-b border-[var(--line)] last:border-none">
                    <td className="px-3 py-2 font-semibold text-[var(--sea-ink)]">{g.name}</td>
            ...
            {data.recentEvents.map((e) => (
              ...
              >
                <span className="text-[var(--sea-ink-soft)]">
                  {formatTime(e.occurredAt)} · {formatDayLabel(e.occurredAt.slice(0, 10))}
                </span>
        ...
        {data.map((count, i) => {
          ...
          return (
            <div
              ...
              title={`${full[i]}: ${count}`}
            >
              <span className="text-[10px] font-semibold text-[var(--sea-ink-soft)]">{count}</span>
              <div
                className="w-full rounded-t-md bg-[var(--lagoon-deep)]"
                style={{ height: `${h}px` }}
              />
              <span className="text-[10px] text-[var(--sea-ink-soft)]">{labels[i]}</span>
            </div>
          )
        })}
      </div>
        ...
        {data.map((count, i) => {
          ...
          return (
            <div
              ...
              title={`${i}:00 – ${count}`}
            >
              <div className="w-full rounded-t-sm bg-[var(--palm)]" style={{ height: `${h}px` }} />
